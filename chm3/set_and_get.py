...
def find_min_eigen(A_band, A_band_fact, eps_a, eps_g, M):
    ...
    for k in range(1, M + 1):
        ...
        # Оценка λ
        Av = matvec(A_band, v)
        lambda_est = dot(v, Av)  # Убрал abs, т.к. SPD >0
        # Невязка относительно λ (для r = max |Av - λ v|)
        residual_vec = [avi - lambda_est * vi for avi, vi in zip(Av, v)]
        r = max(abs(comp) for comp in residual_vec)  # Теперь r = ||Av - λv||_∞ как в замечании

        # Проверка по критериям (используем check_eigen_convergence), пропускаем первую итерацию
        if k > 1 and check_eigen_convergence(lambda_est, prev_lambda, v, prev_v, eps_a, eps_g):
            return 0, lambda_est, v, k, r

        prev_lambda = lambda_est
        prev_v = v[:]  # Сохраняем prev_v
        x = v

    # Если не сошлось
    Av = matvec(A_band, x)
    residual_vec = [avi - lambda_est * vi for avi, vi in zip(Av, x)]
    r = max(abs(comp) for comp in residual_vec)
    return 1, lambda_est, x, M, r
